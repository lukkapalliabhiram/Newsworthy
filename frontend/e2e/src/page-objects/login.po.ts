...
export class LoginPage {
 ...
 // navigate to  note view dashboard
  ...
  // get username and password details
  getInvalidMockLoginDetail(): any {
    const loginDetail: any = { username: 'adminn', password : 'admin'};
    return loginDetail;
  }

   // set username and password input box values
   addInvalidLoginValues(): any {
    const login: any = this.getInvalidMockLoginDetail();
    this.getUserNameInputBox().sendKeys(login.username);
    this.getPasswordInputBox().sendKeys(login.password);
    return Object.keys(login).map(key => login[key]);
  } 

  getInvalidCredentialsErrorMsg(){
    return element(by.css(`strong.error-message`)).getText() as Promise<string>;
  }
}
