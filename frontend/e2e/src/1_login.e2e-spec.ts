...
describe('login page', () => {
  ...
  it('should show error message for invalid credentials', () => {
    page.navigateToLogin();
    let newNoteValues = page.addInvalidLoginValues();
    ...
    page.getCurrentURL().then((url) => {
      expect(url.indexOf('/login') > -1).toBeTruthy();
      page.getInvalidCredentialsErrorMsg().then((msg) => {
        expect(msg.indexOf(`Unauthorized`) > -1).toBeTruthy();
      });
    });
  });
  ...
  it('should login into the system', () => {
    ...
    page.getCurrentURL().then((url) => {
      if (url.indexOf('login') > -1) {
        ...
        page.clickSubmitButton();
        //page.navigateToNoteView();
        expect(page.getCurrentURL()).toContain('dashboard/view/trending', 'Should navigate to headlines view dashboard');
      } else {
        expect(page.getCurrentURL()).toContain('dashboard/view/trending', 'Should navigate to headlines view dashboard');
      }
    });
